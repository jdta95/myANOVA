...
Df: This column contains the degrees of freedom for each variable. For a continuous numeric variable like Sepal.Width, the degrees of freedom will be 1. For a categorical variable like Species, the degrees of freedom is calculated as the number of categories minus 1. The degrees of freedom for the residuals is the number of observations minus 1 minus the parameters' degrees of freedom.

Sum Sq: This column contains sums of squares. In each row, a linear regression model is run with the row name variable and all variables above it. To calculate the sum of squares, first the squared differences between the fitted values and the outcome mean are summed. Then the sum minus the prior sums of squares gives the sums of squares for the current row.
...
## Part 2: 3 or more linear models
